...
useEffect(() => {
  ...
  fabric.Image.fromURL(canvasTemp, (img) => {
    const existingText = fabricCanvas.getObjects().find(obj => obj.type === 'image' && obj.name === 'text');
    if (existingText) {
...
useEffect(() => {
  ...
  fabric.Image.fromURL(numVal, (img) => {
    const existingNumber = fabricCanvas.getObjects().find(obj => obj.type === 'image' && obj.name === 'number');
    if (existingNumber) {
...
useEffect(() => {
  ...
  const existingImage = fabricCanvas.getObjects().find(obj => obj.type === 'image' && obj.name === 'selectedImage');

  if (selectedImage) {
    // If a new image is provided, load and add it
    ...
    // If selectedImage is null, remove the existing image
    if (existingImage) {
      fabricCanvas.remove(existingImage);
      fabricCanvas.renderAll();
    }
